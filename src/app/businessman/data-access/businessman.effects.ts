        ...
        return this.actions$.pipe(
          ...
          mergeMap((action) => {
            return this.businessmanService.insertBusinessman(action.businessman).pipe(
              map(() => BusinessmanActions.insertBusinessmanSuccess({ businessman: action.businessman })),
              catchError(() => of(BusinessmanActions.insertBusinessmanFailed({ businessman: action.businessman })))
            );
          })
        );
      });
      updateBusinessman$ = this.actions$.pipe(
        ...
        mergeMap((action) => {
          return this.businessmanService.updateBusinessman(action.businessman).pipe(
            ...
            catchError(() => of(BusinessmanActions.updateBusinessmanFailed({ businessman: action.businessman })))
          );
        })
      );
